...
deny contains res if {
	...
	block.kind == "resource"
	res := result.new(
		sprintf("raw resource '%s.%s' is forbidden; use a module from ../modules/", [block.type, block.labels[0]]),
		block,
	)
}
...
deny contains res if {
	...
	block.kind == "data"
	res := result.new(
		sprintf("raw data source '%s.%s' is forbidden", [block.type, block.labels[0]]),
		block,
	)
}
...
deny contains res if {
	...
	res := result.new(
		sprintf("module '%s' source '%s' must be under ../modules/", [block.labels[0], src]),
		block,
	)
}
